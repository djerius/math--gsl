...
function and its derivativess, $fdf, and the initial guess, $root. 

$fdf is a hash with the following elements:

=over

=item f

A subroutine returning the value of the function at a given C<x>. It is called as C<&$f($x)>.

=item df

A subroutine returning the value of the derivative of the function with
respect to C<x>. It is called as C<&$df($x)>.

=item fdf

A subroutine returning the value of the function and its derivative with
respect to C<x>. It is called as C<&$fdf($x)>.
...
For example,
  ...
  $fdf = {
    f   => sub { my $x = shift; exp( 2 * $x ) },
    df  => sub { my $x = shift; 2 * exp( 2 * $x ) },
    fdf => sub { my $x = shift;
                 my $t = exp( 2 * x );
                 $t, 2 * $t;
           }
